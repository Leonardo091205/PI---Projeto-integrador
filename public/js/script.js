let cartItems = [];  // Variável de carrinho mantida conforme sua solicitação
...
function removeItemFromCart(index) {
    const itemElement = document.getElementById(`cart-item-${index}`); // Pega o item do carrinho
    itemElement.classList.add('removing'); // Adiciona a animação de remoção

    // Espera a animação de remoção terminar antes de remover o item do carrinho
    setTimeout(() => {
        cartItems.splice(index, 1); // Remove o item da lista
        updateCartMenu(); // Atualiza o carrinho
    }, 1700); // Atraso para permitir que a animação seja concluída
}
...
function updateCartMenu() {
    ...
    if (cartItems.length === 0) {
        ...
        cartItems.forEach((item, index) => {
            ...
            itemElement.id = `cart-item-${index}`; // Adiciona ID único ao item para animá-lo
            itemElement.innerHTML = `
                <img src="${item.imagem}" alt="${item.nome}"> <!-- Imagem do produto -->
                <div>
                    <p class="cart-item-name">${index + 1}. ${item.nome}</p>
                    <p class="cart-item-price">R$ ${item.valor}</p>
    
                </div>
                <button onclick="removeItemFromCart(${index})">X</button> <!-- Botão de remover -->
            `;
...
// Função para alternar o menu do carrinho
...
function initSlider({startAtIndex = 0, autoPlay = true, timeInterval = 8000}) {
    ...
    function scrollCategories(direction) {
        ...
        const categoryBox = document.querySelector(".category-box"); // Seleciona uma categoria para calcular o tamanho
        const visibleBoxes = Math.floor(wrapper.offsetWidth / categoryBox.offsetWidth); // Quantas cabem visíveis
        const scrollAmount = categoryBox.offsetWidth * visibleBoxes; // Calcula o deslocamento com base nas categorias visíveis
